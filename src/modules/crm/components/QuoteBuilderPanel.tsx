...
export function QuoteBuilderPanel({ leadId, dealId, accountId: propAccountId, accountName }: QuoteBuilderPanelProps) {
  ...
  useEffect(() => { triggerAutoSave() }, [selectedAccountId, validUntil, lineItems, discount, terms, notes, triggerAutoSave])
  useEffect(() => { return () => { if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current) } }, [])

  // ── Actions ──

  function handleDownloadQuote() {
    const accountObj = accounts.find((a) => a.id === (propAccountId ?? selectedAccountId))
    const resolvedAccountName = accountName ?? accountObj?.name ?? 'N/A'
    ...
    if (dealId) meta.push(['Deal', dealId])
    ...
    lineItems.forEach((li, idx) => {
      ...
      if (li.bomId && li.bomComponents.length > 0) {
        const included = li.bomComponents.filter((c) => c.action !== 'EXCLUDE')
        const excluded = li.bomComponents.filter((c) => c.action === 'EXCLUDE')
        const lines: string[] = []
        if (included.length) {
          lines.push('Components:')
          included.forEach((c) => {
            const partLabel = c.action === 'SWAP' ? `${c.swapPartName} (replaces ${c.partName})` : c.partName
            lines.push(`  • ${partLabel} × ${c.quantity}`)
          })
        }
        if (excluded.length) {
          lines.push('Removed:')
          excluded.forEach((c) => lines.push(`  ✕ ${c.partName}`))
        }
        if (li.additionalComponents.length) {
          lines.push('Added:')
          li.additionalComponents.forEach((ac) => {
            const label = ac.partId ? `${ac.partName} (${ac.partSku})` : ac.partName
            lines.push(`  + ${label} × ${ac.quantity}`)
          })
        }
        if (li.configNotes) lines.push(`Notes: ${li.configNotes}`)
        if (lines.length) {
          tableBody.push(['', lines.join('\n'), '', '', ''])
        }
      }
    })
