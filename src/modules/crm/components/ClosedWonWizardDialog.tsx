...
import { Check, Plus, Trash2, Building2, ShoppingCart, Upload, FileText, X } from 'lucide-react'
...
function ClosedWonWizardDialog({
  ...
  const isSOValid = lineItems.some((li) => li.item.trim() !== '' && li.qty > 0 && li.rate > 0)
  const isOnSOStep = isLead ? currentStep === 2 : currentStep === 1

  // Line item CRUD
  function updateLineItem(id: string, field: keyof Omit<SOLineItem, 'id'>, value: string | number) {
    ...
    const result: ClosedWonResult = {
      salesOrder: {
        lineItems: lineItems
          ...
          .map(({ item, description, qty, rate }) => ({ item, description, qty, rate })),
        notes,
            ...
            {isLead && (
              <>
                <div className={cn(
                  ...
                  currentStep === 1 ? 'bg-primary/10 text-primary' : 'bg-[#e8fff3] text-[#0b5c22]'
                )}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          ...
          {isOnSOStep && (
            <div className="space-y-5">
              ...
              <div className="flex items-center gap-3 rounded-lg border border-border/60 bg-muted/30 px-4 py-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium">
                    {isLead ? accountName : existingAccountName ?? 'N/A'}
                  </p>
                </div>
              ...
              {/* Categories Interested — Dropdown */}
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    disabled={!orderType || (WARRANTY_OPTIONS[orderType]?.length ?? 0) === 0}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={orderType === 'Rental' ? 'N/A' : 'Select warranty'} />
                    </SelectTrigger>
                    <SelectContent>
                      {(WARRANTY_OPTIONS[orderType] ?? []).map((w) => (
              ...
              <div>
                ...
                <div className="space-y-3">
                  <div className="space-y-1.5">
                    <Label className="text-[13px]">Street</Label>
              ...
              {/* Line Items — compact card-based layout for dialog */}
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-3">
                  Quote / Line Items
                </p>
                ...
                <Button
                  ...
                  size="sm"
                  className="mt-2"
                  onClick={() => setLineItems((prev) => [...prev, createEmptySOItem()])}
                >
